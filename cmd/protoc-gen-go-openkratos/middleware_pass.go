...
func generateMiddlewareFile(gen *protogen.Plugin, file *protogen.File, httpEnabled, grpcEnabled, omitHTTP bool) (*protogen.GeneratedFile, error) {
	...
	if err := validateMiddlewareIdentifiers(gen, file, httpEnabled, grpcEnabled, omitHTTP); err != nil {
		return nil, err
	}
...
func validateMiddlewareIdentifiers(gen *protogen.Plugin, file *protogen.File, httpEnabled, grpcEnabled, omitHTTP bool) error {
	...
	for _, service := range file.Services {
		hasHTTP := httpEnabled && len(enabledHTTPMethods(service, omitHTTP)) > 0
		if !hasHTTP && !grpcEnabled {
			continue
		}
		identifiers := []string{
