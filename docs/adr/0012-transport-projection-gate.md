...
ADR-0006 的分工是：契约错误在 Protobuf 中声明、由 protoc-gen-go-errors 生成 sentinel；`errors.Of()` 供进程内失败使用，不需要声明。但 ADR-0007 落地的 `PublicOf` 对两者一视同仁——**任何** `*errors.Error` 的 Kind、domain、reason、message、metadata 都原样出线。这留下一个 fail-open 缺口，下游应用审计（2026-08-17）给出了两个实测后果 `[一手：下游应用审计]`：

1. **内部 reason 意外冻结为公共 API。** 未经处理的存储层错误漏到边界时，其进程内 reason（如 `store.internal/RESOURCE_NOT_FOUND`）被序列化为客户端可匹配的线上契约。客户端一旦开始匹配它，改名即破坏性变更——一个从未打算公开的标识符被事故性地冻结。
