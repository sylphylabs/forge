...
import (
	...
	bdtest "github.com/sylphylabs/forge/internal/testdata/binding"
	"github.com/sylphylabs/forge/internal/testdata/complex"
	ectest "github.com/sylphylabs/forge/internal/testdata/encoding"
)

// This variable can be replaced with -ldflags like below:
// go test "-ldflags=-X github.com/sylphylabs/forge/encoding/form.tagNameTest=form"
...
func TestFormCodecMarshal(t *testing.T) {
	...
	if !reflect.DeepEqual([]byte("password=forge_pwd&username=forge"), content) {
		t.Errorf("expect %s, got %s", "password=forge_pwd&username=forge", content)
	}
	...
	if !reflect.DeepEqual([]byte("username=forge"), content) {
		t.Errorf("expect %s, got %s", "username=forge", content)
	}
	...
	if !reflect.DeepEqual([]byte("id=1&name=forge"), content) {
		t.Errorf("expect %s, got %s", "id=1&name=forge", content)
	}
}
...
func TestFormCodecUnmarshal(t *testing.T) {
	...
	if !reflect.DeepEqual("forge", bindReq.Username) {
		t.Errorf("expect %v, got %v", "forge", bindReq.Username)
	}
	if !reflect.DeepEqual("forge_pwd", bindReq.Password) {
		t.Errorf("expect %v, got %v", "forge_pwd", bindReq.Password)
	}
}
...
func TestUnmarshal(t *testing.T) {
	...
	want := &LoginRequest{Username: "forge", Password: "forge_pwd"}
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("Unmarshal() = %#v, want %#v", got, want)
	}

	protoGot := new(bdtest.HelloRequest)
	if err := Unmarshal(url.Values{"name": {"forge"}}, protoGot); err != nil {
		t.Fatal(err)
	}
	if protoGot.GetName() != "forge" {
		t.Fatalf("Unmarshal() protobuf name = %q, want forge", protoGot.GetName())
	}
...
func BenchmarkDecodeValue(b *testing.B) {
	b.Run("simple", func(b *testing.B) {
		...
		b.ReportAllocs()
		for b.Loop() {
			if err := DecodeValue(&target, "name", "forge"); err != nil {
				b.Fatal(err)
			}
		}
	})
...
func TestProtoEncodeDecode(t *testing.T) {
	in := &complex.Complex{
		...
		D:       22.22,
		Byte:    []byte("123"),
		Map:     map[string]string{"forge": "https://go-kratos.dev/", "forge_start": "https://go-kratos.dev/en/docs/getting-started/start/"},
	...
	if "a=19&age=18&b=true&bool=false&byte=MTIz&bytes=MTIz&count=3&d=22.22&double=12.33&duration="+
		...
		"map%5Bforge%5D=https%3A%2F%2Fgo-kratos.dev%2F&map%5Bforge_start%5D=https%3A%2F%2Fgo-kratos.dev%2Fen%2Fdocs%2Fgetting-started%2Fstart%2F&"+
...
func TestDecodeStructPb(t *testing.T) {
	...
	query := `data={"name":"forge"}&data_list={"name1": "forge"}&data_list={"name2": "go-kratos"}`
	if err := encoding.GetCodec(Name).Unmarshal([]byte(query), req); err != nil {
		t.Fatal(err)
	}
	if "forge" != req.Data.GetFields()["name"].GetStringValue() {
		t.Errorf("except %v, got %v", "forge", req.Data.GetFields()["name"].GetStringValue())
	}
	if len(req.DataList) != 2 {
		t.Fatalf("except %v, got %v", 2, len(req.DataList))
	}
	if "forge" != req.DataList[0].GetFields()["name1"].GetStringValue() {
		t.Errorf("except %v, got %v", "forge", req.Data.GetFields()["name1"].GetStringValue())
	}
