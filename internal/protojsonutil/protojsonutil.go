// Package protojsonutil centralises the protojson marshal/unmarshal policy for
// packages that already carry a protobuf dependency (config, encoding/form).
// Keeping the option values in one place means a marshalling-policy change
...
// Marshal serialises v as JSON.  When v implements proto.Message the output
...
// Unmarshal deserialises data into v.  When v implements proto.Message the
